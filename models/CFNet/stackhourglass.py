...
class hourglassup(nn.Module):
    ...
    def forward(self, x, feature4, feature5):
        ...
        conv8 = F.relu(self.conv8(conv4) + self.redir2(conv2))
...
class cfnet(nn.Module):
    def __init__(self, maxdisp, eps, itsa, use_concat_volume=False):
    ...
    def forward(self, left, right):
        if self.itsa:
            ...
            right_ = right.clone().detach() 
            right_.requires_grad = True 
            
            self.eval() 
            featL_ = self.feature_extraction(left_)
            gradL = torch.autograd.grad(outputs=featL_["gw4"], inputs=left_, grad_outputs=torch.ones_like(featL_["gw4"]), create_graph=False)
            gradL = gradL[0].clone().detach()  # B,C,H,W
            
            featR_ = self.feature_extraction(right_)
            gradR = torch.autograd.grad(outputs=featR_["gw4"], inputs=right_, grad_outputs=torch.ones_like(featR_["gw4"]), create_graph=False)
            gradR = gradR[0].clone().detach()  # B,C,H,W
            
            gradL = self.grad_norm(gradL)
            gradR = self.grad_norm(gradR)
            
            left_scp = left.clone().detach() + gradL
            right_scp = right.clone().detach() + gradR
            
            left_scp = self.clip(left_scp).detach()
            right_scp = self.clip(right_scp).detach()
            
            del left_, right_ 
            
            self.train()
            
            features_left_scp = self.feature_extraction(left_scp)
            features_right_scp = self.feature_extraction(right_scp)
        
            features_left = self.feature_extraction(left)
            features_right = self.feature_extraction(right)
            
            featEsts = {"left": features_left["gw4"],
                        "right": features_right["gw4"],
                        "left_scp": features_left_scp["gw4"],
                        "right_scp": features_right_scp["gw4"]}
        ...
        maxdisparity_s3 = F.interpolate(maxdisparity_s3 * 2, [left.size()[2] // 4, left.size()[3] // 4], mode='bilinear', align_corners=True)
        mindisparity_s3 = F.interpolate(mindisparity_s3 * 2, [left.size()[2] // 4, left.size()[3] // 4], mode='bilinear',
                                    align_corners=True)
        ...
        maxdisparity_s2 = F.interpolate(maxdisparity_s2 * 2, [left.size()[2] // 2, left.size()[3] // 2], mode='bilinear',
                                     align_corners=True)
        mindisparity_s2 = F.interpolate(mindisparity_s2 * 2, [left.size()[2] // 2, left.size()[3] // 2], mode='bilinear',
                                     align_corners=True)
        ...
        if self.training:
            ...
            cost0_4 = F.interpolate(cost0_4, [self.maxdisp, left.size()[2], left.size()[3]], mode='trilinear', align_corners=True)
            cost0_4 = torch.squeeze(cost0_4, 1)
            pred0_4 = F.softmax(cost0_4, dim=1)
            pred0_4 = disparity_regression(pred0_4, self.maxdisp)

            cost1_4 = F.interpolate(cost1_4, [self.maxdisp, left.size()[2], left.size()[3]], mode='trilinear', align_corners=True)
            cost1_4 = torch.squeeze(cost1_4, 1)
            pred1_4 = F.softmax(cost1_4, dim=1)
            pred1_4 = disparity_regression(pred1_4, self.maxdisp)

            pred2_s4 = F.interpolate(pred2_s4 * 8, [left.size()[2], left.size()[3]], mode='bilinear', align_corners=True)
            ...
            pred0_s3 = F.interpolate(pred0_s3 * 4, [left.size()[2], left.size()[3]], mode='bilinear',
                                     align_corners=True)
            ...
            predmid_s3 = F.interpolate(predmid_s3 * 4, [left.size()[2], left.size()[3]], mode='bilinear',
                                     align_corners=True)
            predmid_s3 = torch.squeeze(predmid_s3, 1)

            pred1_s3_up = F.interpolate(pred1_s3 * 4, [left.size()[2], left.size()[3]], mode='bilinear', align_corners=True)
            ...
            pred0_s2 = F.interpolate(pred0_s2 * 2, [left.size()[2], left.size()[3]], mode='bilinear', align_corners=True)
            ...
            predmid_s2 = F.interpolate(predmid_s2 * 4, [left.size()[2], left.size()[3]], mode='bilinear',
                                     align_corners=True)
            predmid_s2 = torch.squeeze(predmid_s2, 1)

            pred1_s2 = F.interpolate(pred1_s2 * 2, [left.size()[2], left.size()[3]], mode='bilinear', align_corners=True)
            pred1_s2 = torch.squeeze(pred1_s2, 1)

            if self.itsa:
                return featEsts, [pred0_4, pred1_4, pred2_s4, pred0_s3, predmid_s3, pred1_s3_up, pred0_s2, predmid_s2, pred1_s2]
            ...
            pred1_s3_up = F.interpolate(pred1_s3 * 4, [left.size()[2], left.size()[3]], mode='bilinear',
                                     align_corners=True)
            pred1_s3_up = torch.squeeze(pred1_s3_up, 1)
...
def CFNet(d, eps, itsa):
    return cfnet(d, eps=eps, use_concat_volume=True, itsa=itsa)